...
TAC.LogBanToDiscord = function (data)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = 'PanvasiAC | ' .. TAC.Version
        }
    }

    PerformHttpRequest(TAC.Config.Webhook, function(err, text, headers) end, 'POST', json.encode({ username = 'PanvasiAC', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
